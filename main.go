...
import (
	...
	_ "github.com/Vesuvy/feature-service/docs"
...
func main() {
	...
	r.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:5173", "http://localhost:8080"}, // Разрешить запросы с фронтенда
	...
	*/
	...
	v1 := r.Group("/api/v1", middleware.AuthMiddleware())
	{
		...
		admin := v1.Group("/admin")
		{
			...
			// теги и пользователя
			r.POST("/users/:userID/tags", func(c *gin.Context) {
				handlers.AssignTagsToUserHandler(c, dbStruct)
			})
			r.POST("/features/:featureID/tags/:tagID/enable", func(c *gin.Context) {
				handlers.ToggleFeatureForTagsHandler(c, dbStruct)
			})
			r.GET("/features/:featureID/active-users", func(c *gin.Context) {
				handlers.GetActiveUsersForFeatureHandler(c, dbStruct)
			})
		}
